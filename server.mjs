...
const EMPTY = () => ({
  schema: 6,
  users: [],          // { id, name, active, joinedAt, salt, hash }
...
function questionReady(q) {
  ...
  return !!q.text.trim() && filled.length >= MIN_OPTIONS
    && Number.isInteger(q.correct) && !!(q.options[q.correct] || '').trim();
}
...
function advance(live, qCount) {
  ...
  if (phase === 'tb') { live.phase = 'gap'; return; }
...
function back(live, qCount) {
  ...
  if (phase === 'tb') { live.phase = 'q'; live.index = qCount - 1; return; }
  if (phase === 'gap') { live.phase = 'tb'; return; }
  if (phase === 'a') {
    if (index > 0) live.index--; else live.phase = 'gap';
    return;
  }
  if (phase === 'tba') { live.phase = 'a'; live.index = qCount - 1; return; }
...
function deriveRoles(rows) {
  if (!rows.length) return { quizMasterId: null, topicPickerId: null, reason: {} };
  const last = rows[rows.length - 1];
  const second = rows.length > 1 ? rows[rows.length - 2] : rows[0];
...
function visibleQuiz(quiz, canSeeAnswers, canSeeTopic) {
  if (!quiz) return null;
  ...
  return {
    ...
    questions: quiz.questions.map((q) => ({
      id: q.id, text: q.text, options: q.options, media: q.media || null
    })),
    tieBreaker: {
...
function stateFor(userId) {
  ...
  const live = db.live;
  const canSeeAnswers = isMaster || answersRevealed(live);
  // The topic is a surprise for the rest of the team: the picker chose it and
  // the quiz master has to write to it, nobody else needs to know. Once a game
  // is running it is up on the big screen anyway, so it stops being a secret.
  const canSeeTopic = isMaster || isPicker || !!live;
  ...
  const out = {
    ...
    upcoming: db.upcoming ? {
      ...
      quiz: visibleQuiz(db.upcoming.quiz, canSeeAnswers, canSeeTopic),
  ...
  if (live) {
    ...
    out.live = {
      ...
      phase: live.phase,
      index: live.index,
      topic: live.final ? live.final.topic : (db.upcoming ? db.upcoming.topic : ''),
...
function ensureUpcoming() {
  if (db.upcoming || db.users.length < 2) return;
  const [a, b] = db.users;
  db.upcoming = {
...
async function api(req, res, path) {
  ...
  if (path === '/api/quiz' && req.method === 'PUT') {
    ...
    quiz.questions = quiz.questions.slice(0, MAX_QUESTIONS).map((q) => {
      ...
      const correct = Number.isInteger(q.correct) && q.correct >= 0 && q.correct < options.length
        ? q.correct : null;
      return { ...q, options, correct, media: cleanMedia(q.media) };
    });
  ...
  if (path === '/api/live/start' && req.method === 'POST') {
    ...
    db.live = {
      ...
      players: db.users.filter((u) => u.active !== false).map((u) => u.id),
  ...
  if (path === '/api/live/tiebreak' && req.method === 'POST') {
    ...
    if (me === db.upcoming.quizMasterId) return json(res, 403, { error: 'You wrote this one' });
    if (!live.players.includes(me)) live.players.push(me);
    live.tieGuesses[me] = Number(value);
    broadcast();
    return json(res, 200, { ok: true });
  }
  ...
  if (path === '/api/live/finish' && req.method === 'POST') {
    ...
    const after = new Date(u.date + 'T12:00:00');
